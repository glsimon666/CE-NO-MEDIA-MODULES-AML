...
#define MMU_COPY_IDX(n)		(FB_LOOP_BUF_COUNT + BUF_POOL_SIZE + n + 1)
...
#define MMU_COPY_IDX(n)	(FB_LOOP_BUF_COUNT + BUF_POOL_SIZE * 2 + n + 1)
#endif
...
#define DEC_RESULT_ERROR_DATA       16
...
static int front_decpic_done_update(struct hevc_state_s *hevc, uint8_t reset_flag)
{
	...
	if ((hevc->PB_skip_mode == 0)
		...
		int index = find_near_pic_index(hevc, hevc->cur_pic);
		...
		if (pic != NULL) {
			...
			hevc_print(hevc, H265_DEBUG_BUFMGR,
				...
				cur_pic->POC, pic->POC, cur_pic->lcu_cnt, cur_pic->decoder_tile_cnt);
		}
	}
...
static void init_mv_buf_list(struct hevc_state_s *hevc)
{
	int i;
	int new_size;
	int ref_pic_num = get_work_pic_num(hevc);
		...
		if (buf_size > 0) {
			...
			if (ret >= 0) {
				...
				if (!vdec_secure(hw_to_vdec(hevc)) && (hevc->m_BUF[i].start_adr)) {
					void *mem_start_virt = codec_mm_phys_to_virt(hevc->m_BUF[i].start_adr);
					int value = 0;
					int dw_mode = get_double_write_mode(hevc);

					if ((dw_mode == 0x10) && (hevc->PB_skip_mode == 0)) {
						value = 0x80;
					}

					if (mem_start_virt) {
						memset(mem_start_virt, value, hevc->m_BUF[i].size);
						codec_mm_dma_flush(mem_start_virt,
							hevc->m_BUF[i].size, DMA_TO_DEVICE);
					} else {
						codec_mm_memset(hevc->m_BUF[i].start_adr,
							value, hevc->m_BUF[i].size);
					}
				}
			}
		} else
...
#endif
		PR_FILL("output_ready:%d, mv_wr_start %x vf_ref %d need_mmu_copy %d",
		...
		unsigned char flush_flag)
{
	...
	if (hevc->i_only & 0x4) {
		...
		if ((hevc->front_back_mode == 0) && pic_display
...
static int alloc_default_ref_buffer(struct hevc_state_s *hevc)
{
	...
	int dw_mode = get_double_write_mode(hevc);
	int y_buf_size = pic->dw_u_v_adr - pic->dw_y_adr;
	int buf_size = cal_current_buf_size(hevc, NULL);
	...
	if ((hevc->ref_pic.dw_y_adr == 0) && (buf_size != 0)) {
		...
		if ((buf_size > 0) && (dw_mode == 16)) {
			...
			if (ret < 0) {
				...
				hevc_print(hevc, H265_DEBUG_BUFMGR,
					"Default Ref Buffer: dw_y_adr %p size:%d dw_uv_adr %p size\n",
	...
	unsigned char dv_meta_flag)
{
	...
	if ((pic->BUF_index < 0) || (!hevc->m_BUF[pic->BUF_index].start_adr)) {
		pr_info("%s, star_addr null, aux data buf released\n", __func__);
		return;
	}
		...
		bool check_dma, struct PIC_s *pic)
{
	...
	hevc_print(hevc,
			...
			"%s pic index %d poc %d scatter_alloc %d page_start used_4k_num %d used_4k_num1 %d\n",
			"decoder_mmu_box_free_idx_tail",
			pic->index,
			pic->POC,
			pic->scatter_alloc,
			used_4k_num, used_4k_num1);
	if (check_dma)
		hevc_mmu_dma_check(hw_to_vdec(hevc));

	decoder_mmu_box_free_idx_tail(hevc->mmu_box,
		pic->index, used_4k_num);
#ifdef NEW_FB_CODE
	if (hevc->front_back_mode)
		decoder_mmu_box_free_idx_tail(hevc->mmu_box_1,
			pic->index, used_4k_num1);
#endif
...
#ifdef NEW_FB_CODE
		if (hevc->front_back_mode)
			decoder_mmu_box_free_idx_tail(hevc->mmu_box_dw_1,
				pic->index, used_4k_num1);
#endif
	}
#endif
...
static void get_picture_qos_info(struct hevc_state_s *hevc, bool back_flag)
{
	...
	struct vdec_s *vdec = hw_to_vdec(hevc);
	struct vframe_qos_s vqos_0 = {0}, vqos_1 = {0};
...
//
void ateme_parse_sei(struct hevc_state_s *hevc, struct PIC_s *pic,
	...
	pic->sei_present_flag |= SEI_MASTER_DISPLAY_COLOR_MASK;
	...
	p_sei += 2;
	pic->sei_present_flag |= SEI_CONTENT_LIGHT_LEVEL_MASK;
}
		...
		while (p < pic->aux_data_buf
			...
			if (type == 0x02000000) {
				...
				if (size > 42) // check for ateme
					ateme_parse_sei(hevc, pic, p, size);
			}
			p += size;
		}
	}
...
static struct vframe_s *vh265_vf_get(void *op_arg)
{
	...
	if (hevc->front_back_mode) {
		if (kfifo_peek(&hevc->display_q, &vf) && vf) {
				...
				if (index < MAX_REF_PIC_NUM) {
					pic = hevc->m_PIC[index];
					if (pic && (!pic->back_done_mark || pic->need_mmu_copy))
						return NULL;
				}
		} else
			return NULL;
...
#ifdef NEW_FB_CODE
			fill_frame_info(hevc, pic, pic->stream_size, vf->pts);
	...
	if (((get_dbg_flag(hevc) &
		...
		if (hevc->decode_timeout_count_back == 0)
			timeout_process_back(hevc);
				...
				(READ_VREG(HEVC_DEC_STATUS_DBE) != HEVC_BE_DECODE_DATA_DONE)) {
		hevc_print(hevc, 0, "timer_back fast check timeout\n");
...
static void timeout_process_back(struct hevc_state_s *hevc)
{
	...
	hevc_print(hevc, 0, "%s decoder timeout HEVC_MPC_E_DBE=0x%x\n",
...
void get_mmu_copy_map(struct hevc_state_s* hevc, PIC_t* pic, u32 page_num)
{
	int i = 0;
	...
	struct vdec_s *vdec,int from)
{
	...
	if (debug & H265_DEBUG_BUFMGR_MORE) {
		for (i = 0; i < MAX_REF_PIC_NUM; i++) {
			...
			if (ref_pic == NULL)
				break;
			hevc_print(hevc, 0, "pic index %d, ref index %d ref_pic backend_ref %d, %d\n",
				pic->index, ref_pic->index, ref_pic->backend_ref, __LINE__);
		}
	}
...
static void vh265_dump_state(struct vdec_s *vdec)
{
	...
	hevc_print(hevc, 0,
		"front_back_mode (%d), is_framebase(%d), eos %d, dec_result 0x%x dec_frm %d disp_frm %d run %d not_run_ready %d input_empty %d, error_frame_count %d, drop_frame_count %d, start_process_time %lu, start_process_time_back %lu\n",
		hevc->front_back_mode,
		input_frame_based(vdec),
		hevc->eos,
		hevc->dec_result,
		...
		hevc->gvs->error_frame_count,
		hevc->gvs->drop_frame_count,
		hevc->start_process_time,
		hevc->start_process_time_back
		);
...
static int __init amvdec_h265_driver_init_module(void)
{
	...
	register_set_debug_flag_func(DEBUG_AMVDEC_H265_FB, set_debug_flag);
	...
	vcodec_profile_register_v2("hevc_fb", VFORMAT_HEVC, 0);
