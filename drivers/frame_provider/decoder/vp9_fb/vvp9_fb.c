 ...
 *
 *#define VP9D_MPP_REFINFO_TBL_ACCCONFIG_DBE1			0x3942
 *#define VP9D_MPP_REFINFO_DATA_DBE1					0x3943
 *#define VP9D_MPP_REF_SCALE_ENBL_DBE1				0x3941
 *#endif
 *
 */
...
#define DRIVER_NAME "amvdec_vp9_fb"
#define DRIVER_HEADER_NAME "amvdec_vp9_header_fb"
...
#else
#define LMEM_DUMP_ADR_DBE         HEVC_ASSIST_SCRATCH_X
#endif
...
#define HEVC_DEC_IDLE  VP9_10B_DEC_IDLE
...
#define VP9_DEBUG_DUAL_CORE              0x4000000
...
#ifdef NEW_FRONT_BACK_CODE
	int mmu_alloc_flag;
	struct PIC_BUFFER_CONFIG_s *pic_refs[REFS_PER_FRAME];
	uint8_t depth;
	int backend_ref;
#endif
...
#define MAX_BMMU_BUFFER_NUM (FRAME_BUFFERS + HEADER_FRAME_BUFFERS + 1 + FB_BUFFER_NUM)
...
#ifdef NEW_FB_CODE
#define FB_BUF_ID (WORK_SPACE_BUF_ID + 1)
#endif
#else
#define MAX_BMMU_BUFFER_NUM \
	(FRAME_BUFFERS + HEADER_FRAME_BUFFERS + MV_BUFFER_NUM + 1 + FB_BUFFER_NUM)
#define VF_BUFFER_IDX(n) (n)
#define HEADER_BUFFER_IDX(n) (FRAME_BUFFERS + n)
...
void vp9_loop_filter_frame_init(struct VP9Decoder_s *pbi, struct segmentation *seg,
		struct loop_filter_info_n *lfi, struct loopfilter *lf,
		int default_filt_lvl);
void vp9_loop_filter_frame_init_fb(struct VP9Decoder_s *pbi, struct segmentation *seg,
		struct loop_filter_info_n *lfi, struct loopfilter *lf, int default_filt_lvl);
...
//#endif
#ifdef NEW_FRONT_BACK_CODE
	struct buff_s  fb_mpred_imp0;
	struct buff_s  fb_mpred_imp1;
	struct buff_s  tile_header_param;
#endif
	struct buff_s rpm;
	struct buff_s lmem;
#ifdef NEW_FRONT_BACK_CODE
	struct buff_s frame_header_lmem;
	struct buff_s ipp0;
	struct buff_s ipp1;
#endif
...
//pyx cheked but need test
#define	BMMU_IFBUF_LMEM0_ID		(FB_BUF_ID)
#define	BMMU_IFBUFF_MPRED_IMP0_ID 	(BMMU_IFBUF_LMEM0_ID + 1)
...
#define PROC_STATE_SENDAGAIN	2
	...
	void *frame_mmu_dw_map_addr_1;
	dma_addr_t frame_mmu_dw_map_phy_addr_1;
	...
	uint8_t fb_wr_pos;
	uint8_t fb_rd_pos;
	struct buff_s fb_buf_mmu0;
	struct buff_s fb_buf_mmu1;
	struct buff_s fb_buf_sys_imem;
	struct buff_s fb_buf_lmem0;
	struct buff_s fb_buf_mpred_imp0;
	struct buff_s fb_buf_mpred_imp1;
	uint32_t frontend_decoded_count;
	uint32_t backend_decoded_count;
	buff_ptr_t fr;
	buff_ptr_t bk;
	buff_ptr_t next_bk[MAX_FB_IFBUF_NUM];
	struct PIC_BUFFER_CONFIG_s* next_be_decode_pic[MAX_FB_IFBUF_NUM];
	union param_u params[MAX_FB_IFBUF_NUM];
	uint32_t vp9_segment_data[8];
	uint32_t sys_imem_ptr;
	void *sys_imem_ptr_v;
...
#ifdef NEW_FRONT_BACK_CODE
void print_hevc_b_data_path_monitor(int frame_count);
...
#define vp9_print(pbi, flag, fmt, args...)					\
	do {									\
		if (pbi == NULL ||    \
			(flag == 0) || \
			((debug_mask & \
...
#ifdef NEW_FB_CODE
static void reset_process_time_back(struct VP9Decoder_s *pbi)
{
	if (pbi->start_process_time_back) {
		unsigned int process_time =
			1000 * (jiffies - pbi->start_process_time_back) / HZ;
		pbi->start_process_time_back = 0;
		if (process_time > max_process_time_back[pbi->index])
			max_process_time_back[pbi->index] = process_time;
	}
}

static void start_process_time_back(struct VP9Decoder_s *pbi)
{
	pbi->start_process_time_back = jiffies;
	pbi->decode_timeout_count_back = 2;
}

static void timeout_process_back(struct VP9Decoder_s *pbi)
{
	pbi->timeout_num_back++;

	if (pbi->front_back_mode == 1) {
		WRITE_VREG(pbi->backend_ASSIST_MBOX0_MASK, 0);
		amhevc_stop_b();
	}
	...
	pbi->dec_back_result = DEC_BACK_RESULT_TIMEOUT;
...
{
	u32 valid_dw_mode = get_valid_double_write_mode(pbi) & 0xffff;
	u32 dw;
	int w, h;
...
static int get_triple_write_mode(struct VP9Decoder_s *pbi)
{
	int tw = ((triple_write_mode & 0x80000000) == 0) ?
		pbi->triple_write_mode : (triple_write_mode & 0x7fffffff);
...
static __inline__ bool is_dw_p010(struct VP9Decoder_s *pbi)
{
	...
	return ((pbi->double_write_mode & 0x10000) || a);
}
...
static __inline__ bool is_tw_p010(struct VP9Decoder_s *pbi)
{
	...
	return ((pbi->triple_write_mode & 0x10000) || a);
}
...
int vp9_bufmgr_process(struct VP9Decoder_s *pbi, union param_u *params)
{
...
#ifdef NEW_FRONT_BACK_CODE
		&& (frame_bufs[cm->new_fb_idx].buf.backend_ref == 0)
#endif
		) {
		vp9_release_frame_buffer
	...
	pool->frame_bufs[cm->new_fb_idx].buf.back_done_mark = 0;
...
#ifdef NEW_FB_CODE
	pbi->start_process_time_back = 0;
	pbi->decode_timeout_count_back = 0;
	pbi->timeout_num_back = 0;
#endif
...
#ifdef NEW_FRONT_BACK_CODE
#define RPM_BUF_SIZE_FB (0x400 * 4)
#endif
...
static void WRITE_BACK_RET(struct VP9Decoder_s *pbi)
{
...
static void WRITE_BACK_8(struct VP9Decoder_s *pbi, uint32_t spr_addr, uint8_t data)
{
...
static void WRITE_BACK_16(struct VP9Decoder_s *pbi, uint32_t spr_addr, uint8_t rd_addr, uint16_t data)
{
	//spr_addr = ((spr_addr - DOS_BASE_ADR) >> 0) & 0xfff;
	spr_addr = (dos_reg_compat_convert(spr_addr) & 0xfff);
...
static void WRITE_BACK_32(struct VP9Decoder_s *pbi, uint32_t spr_addr, uint32_t data)
{
	//spr_addr = ((spr_addr - DOS_BASE_ADR) >> 0) & 0xfff;
	spr_addr = (dos_reg_compat_convert(spr_addr) & 0xfff);
...
static void READ_INS_WRITE(struct VP9Decoder_s *pbi, uint32_t spr_addr0, uint32_t spr_addr1, uint8_t rd_addr, uint8_t position, uint8_t size)
{
...
static void READ_WRITE_DATA16(struct VP9Decoder_s *pbi, uint32_t spr_addr, uint16_t data, uint8_t position, uint8_t size)
{
	//spr_addr = ((spr_addr - DOS_BASE_ADR) >> 0) & 0xfff;
	spr_addr = (dos_reg_compat_convert(spr_addr) & 0xfff);
...
static void config_hevc_irq_num(struct VP9Decoder_s *pbi)
{
	vp9_print(pbi, VP9_DEBUG_DUAL_CORE, "line: %d\n", __LINE__);
#ifdef NEW_FB_CODE
	if (get_cpu_major_id() >= AM_MESON_CPU_MAJOR_ID_S5) {
		pbi->ASSIST_MBOX0_IRQ_REG = EE_ASSIST_MBOX0_IRQ_REG;
		pbi->ASSIST_MBOX0_CLR_REG = EE_ASSIST_MBOX0_CLR_REG;
		pbi->ASSIST_MBOX0_MASK    = EE_ASSIST_MBOX0_MASK;
		pbi->backend_ASSIST_MBOX0_IRQ_REG = HEVC_ASSIST_MBOX0_IRQ_REG;
		pbi->backend_ASSIST_MBOX0_CLR_REG = HEVC_ASSIST_MBOX0_CLR_REG;
		pbi->backend_ASSIST_MBOX0_MASK    = HEVC_ASSIST_MBOX0_MASK;
	} else {
		pbi->ASSIST_MBOX0_IRQ_REG = HEVC_ASSIST_MBOX0_IRQ_REG;
		pbi->ASSIST_MBOX0_CLR_REG = HEVC_ASSIST_MBOX0_CLR_REG;
		pbi->ASSIST_MBOX0_MASK    = HEVC_ASSIST_MBOX0_MASK;
		pbi->backend_ASSIST_MBOX0_IRQ_REG = EE_ASSIST_MBOX0_IRQ_REG;
		pbi->backend_ASSIST_MBOX0_CLR_REG = EE_ASSIST_MBOX0_CLR_REG;
		pbi->backend_ASSIST_MBOX0_MASK    = EE_ASSIST_MBOX0_MASK;
	}
#else
	pbi->ASSIST_MBOX0_IRQ_REG = HEVC_ASSIST_MBOX0_IRQ_REG;
	pbi->ASSIST_MBOX0_CLR_REG = HEVC_ASSIST_MBOX0_CLR_REG;
...
void vp9_hw_init(struct VP9Decoder_s *pbi, int first_flag, int front_flag, int back_flag);

static void config_bufstate_front_hw(struct VP9Decoder_s *pbi)
{
...
//config lmem buffers
	...
	WRITE_VREG(HEVC_ASSIST_RING_F_START, pbi->fb_buf_lmem0.buf_start);
	WRITE_VREG(HEVC_ASSIST_RING_F_END, pbi->fb_buf_lmem0.buf_end);
	WRITE_VREG(HEVC_ASSIST_RING_F_WPTR, pbi->fr.lmem0_ptr);
	//WRITE_VREG(HEVC_ASSIST_RING_F_RPTR, pbi->bk.lmem0_ptr);
	vp9_print(pbi, VP9_DEBUG_DUAL_CORE, "pbi->fr.lmem0_ptr 0x%x, pbi->bk.lmem0_ptr 0x%x\n", pbi->fr.lmem0_ptr, pbi->bk.lmem0_ptr);
...
static void config_bufstate_back_hw(struct VP9Decoder_s *pbi)
{
...
// config lmem buffers
	...
	WRITE_VREG(HEVC_ASSIST_RING_B_START, pbi->fb_buf_lmem0.buf_start);
...
static void print_loopbufs_ptr(struct VP9Decoder_s *pbi, char* mark, buff_ptr_t* ptr)
{
	vp9_print(pbi, VP9_DEBUG_DUAL_CORE, "==%s==:\n", mark);
	vp9_print(pbi, VP9_DEBUG_DUAL_CORE,
		"mmu0_ptr 0x%x, mmu1_ptr 0x%x, lmem0_ptr 0x%x, mpred_imp0_ptr 0x%x, mpred_imp1_ptr 0x%x\n",
		ptr->mmu0_ptr,
		ptr->mmu1_ptr,
		ptr->lmem0_ptr,
		ptr->mpred_imp0_ptr,
...
//pyx check done
//int32_t default_filt_lvl;

static void config_loop_filter_hw(struct VP9Decoder_s *pbi, union param_u *param)
{
	int i;
	...
	*/
	//printk("###### Loop Filter Param ######\n", frame_count);
	vp9_print(pbi, VP9_DEBUG_DUAL_CORE, "  mode_ref_delta_enabled : %x\n", param->p.mode_ref_delta_enabled);
	vp9_print(pbi, VP9_DEBUG_DUAL_CORE, "  sharpness_level : %x\n", param->p.sharpness_level);
	for (i = 0; i < 4; i++) vp9_print(pbi, VP9_DEBUG_DUAL_CORE, "  ref_deltas[%d] : %x\n", i, param->p.ref_deltas[i]);
	for (i = 0; i < 2; i++) vp9_print(pbi, VP9_DEBUG_DUAL_CORE, "  mode_deltas[%d] : %x\n", i, param->p.mode_deltas[i]);
	vp9_print(pbi, VP9_DEBUG_DUAL_CORE, "  filter_level : %x\n", param->p.filter_level);
	vp9_print(pbi, VP9_DEBUG_DUAL_CORE, "  seg_enabled : %x\n", param->p.seg_enabled);
	vp9_print(pbi, VP9_DEBUG_DUAL_CORE, "  seg_abs_delta : %x\n", param->p.seg_abs_delta);
	for (i=0;i<MAX_SEGMENTS;i++) vp9_print(pbi, VP9_DEBUG_DUAL_CORE, "  seg_lf_info[%d] : %x\n", i, param->p.seg_lf_info[i]);
	vp9_print(pbi, VP9_DEBUG_DUAL_CORE, "  width : %x\n", param->p.width);

	lf->mode_ref_delta_enabled = param->p.mode_ref_delta_enabled;
	lf->sharpness_level = param->p.sharpness_level;
	for (i = 0; i < 4; i++) lf->ref_deltas[i] = param->p.ref_deltas[i];
	for (i = 0; i < 2; i++) lf->mode_deltas[i] = param->p.mode_deltas[i];
	//default_filt_lvl = param->p.filter_level;
	seg_4lf->enabled = param->p.seg_enabled;
	seg_4lf->abs_delta = param->p.seg_abs_delta;
	for (i=0;i<MAX_SEGMENTS;i++) seg_4lf->feature_mask[i] = (param->p.seg_lf_info[i] & 0x8000) ? (1 << SEG_LVL_ALT_LF) : 0;
	for (i=0;i<MAX_SEGMENTS;i++) seg_4lf->feature_data[i][SEG_LVL_ALT_LF] =
		(param->p.seg_lf_info[i] & 0x100) ? -(param->p.seg_lf_info[i] & 0x3f) : (param->p.seg_lf_info[i] & 0x3f);

	// Set pipeline mode
	//lpf_data32 = READ_VREG(HEVC_DBLK_CFGB);
	if (param->p.width >= 1280) lpf_data32 = 1; // dblk pipeline mode=1 for performance
	else                        lpf_data32 = 0;
	//WRITE_VREG(HEVC_DBLK_CFGB, lpf_data32);
	READ_WRITE_DATA16(pbi, HEVC_DBLK_CFGB, lpf_data32, 4, 4);

	/*
	* Update loop filter Thr/Lvl table for every frame
	*/
		vp9_print(pbi, VP9_DEBUG_DUAL_CORE,"[test.c] vp9_loop_filter_frame_init (run before every frame decoding start)\n");
	vp9_loop_filter_frame_init_fb(pbi, seg_4lf, lfi, lf, param->p.filter_level);
}
...
static void config_sao_hw_fb(struct VP9Decoder_s *pbi)
{
	struct VP9_Common_s *cm = &pbi->common;
	...
	vp9_print(pbi, VP9_DEBUG_DUAL_CORE, "[config_sao_hw] lcu_total : %d\n", pic_config->lcu_total);
	vp9_print(pbi, VP9_DEBUG_DUAL_CORE, "[config_sao_hw] mc_y_adr : 0x%x\n", pic_config->mc_y_adr);
	vp9_print(pbi, VP9_DEBUG_DUAL_CORE, "[config_sao_hw] mc_u_v_adr : 0x%x\n", pic_config->mc_u_v_adr);
	vp9_print(pbi, VP9_DEBUG_DUAL_CORE, "[config_sao_hw] header_adr : 0x%x\n", pic_config->header_adr);

#ifdef LOSLESS_COMPRESS_MODE
//pyx wait rain 265 modify
	WRITE_BACK_32(pbi, HEVC_SAO_Y_START_ADDR, pic_config->dw_y_adr);
	if (pbi->mmu_enable) {
		WRITE_BACK_32(pbi, HEVC_CM_HEADER_START_ADDR, pic_config->header_adr);
	}
	if (pbi->dw_mmu_enable) {
		WRITE_BACK_32(pbi, HEVC_CM_HEADER_START_ADDR2, pic_config->header_dw_adr);
	}

#else
	WRITE_BACK_32(pbi, HEVC_SAO_Y_START_ADDR, pic_config->mc_y_adr);
#endif
	vp9_print(pbi, VP9_DEBUG_DUAL_CORE, "[config_sao_hw] sao_body_addr:%x\n", pic_config->mc_y_adr);
	//pyx printk("[config_sao_hw] sao_header_addr:%x\n", pic_config->mc_y_adr + losless_comp_body_size );
	...
	WRITE_BACK_32(pbi, HEVC_SAO_Y_LENGTH ,data32);

#ifndef LOSLESS_COMPRESS_MODE
	WRITE_BACK_32(pbi, HEVC_SAO_C_START_ADDR, pic_config->mc_u_v_adr);
#else
	WRITE_BACK_32(pbi, HEVC_SAO_C_START_ADDR,pic_config->dw_u_v_adr);
#endif

	data32 = (mc_buffer_size_u_v_h<<16);
	WRITE_BACK_32(pbi, HEVC_SAO_C_LENGTH  ,data32);

#ifndef LOSLESS_COMPRESS_MODE
	/* multi tile to do... */
	WRITE_BACK_32(pbi, HEVC_SAO_Y_WPTR , pic_config->mc_y_adr);

	WRITE_BACK_32(pbi, HEVC_SAO_C_WPTR , pic_config->mc_u_v_adr);
#else
	WRITE_BACK_32(pbi, HEVC_SAO_Y_WPTR ,pic_config->dw_y_adr);
	WRITE_BACK_32(pbi, HEVC_SAO_C_WPTR ,pic_config->dw_u_v_adr);
#endif

	if (pbi->dw_mmu_enable) {
		WRITE_BACK_32(pbi, HEVC_DW_VH0_ADDDR, DOUBLE_WRITE_VH0_TEMP);
		WRITE_BACK_32(pbi, HEVC_DW_VH1_ADDDR, DOUBLE_WRITE_VH1_TEMP);
	}
	...
	READ_WRITE_DATA16(pbi, HEVC_SAO_CTRL1, MEM_MAP_MODE, 12, 2);
	//data32 &= (~0x3);
	//data32 |= 0x1; // [1]:dw_disable [0]:cm_disable
	//WRITE_VREG(HEVC_SAO_CTRL1, data32);
	READ_WRITE_DATA16(pbi, HEVC_SAO_CTRL1, 0x1, 0, 2);

	//data32 = READ_VREG(HEVC_SAO_CTRL1_DBE1);
	//data32 &= (~0x3000);
	//data32 |= (MEM_MAP_MODE << 12); // [13:12] axi_aformat, 0-Linear, 1-32x32, 2-64x32
	READ_WRITE_DATA16(pbi, HEVC_SAO_CTRL1_DBE1, MEM_MAP_MODE, 12, 2);
	//data32 &= (~0x3);
	//data32 |= 0x1; // [1]:dw_disable [0]:cm_disable
	//WRITE_VREG(HEVC_SAO_CTRL1_DBE1, data32);
	READ_WRITE_DATA16(pbi, HEVC_SAO_CTRL1_DBE1, 0x1, 0, 2);

	//data32 = READ_VREG(HEVC_SAO_CTRL5); // [23:22] dw_v1_ctrl [21:20] dw_v0_ctrl [19:18] dw_h1_ctrl [17:16] dw_h0_ctrl
	//data32 &= ~(0xff << 16);				 // set them all 0 for VP9_NV21 (no down-scale)
	...
	READ_WRITE_DATA16(pbi, HEVC_SAO_CTRL5_DBE1, 0, 16, 8);

	//data32 = READ_VREG(HEVCD_IPP_AXIIF_CONFIG);
	//data32 &= (~0x30);
	//data32 |= (MEM_MAP_MODE << 4); // [5:4]	 -- address_format 00:linear 01:32x32 10:64x32
	//WRITE_VREG(HEVCD_IPP_AXIIF_CONFIG, data32);
	READ_WRITE_DATA16(pbi, HEVCD_IPP_AXIIF_CONFIG, MEM_MAP_MODE, 4, 2);

	//data32 = READ_VREG(HEVCD_IPP_AXIIF_CONFIG_DBE1);
	//data32 &= (~0x30);
	//data32 |= (MEM_MAP_MODE << 4); // [5:4]	 -- address_format 00:linear 01:32x32 10:64x32
	//WRITE_VREG(HEVCD_IPP_AXIIF_CONFIG_DBE1, data32);
	READ_WRITE_DATA16(pbi, HEVCD_IPP_AXIIF_CONFIG_DBE1, MEM_MAP_MODE, 4, 2);
#else
// m8baby test1902
	//data32 = READ_VREG(HEVC_SAO_CTRL1);
	//data32 &= (~0x3000);
	...
	READ_WRITE_DATA16(pbi, HEVC_SAO_CTRL1, 0x880, 0, 12);
	//data32 &= (~0x3);
	//data32 |= 0x1; // [1]:dw_disable [0]:cm_disable
	//WRITE_VREG(HEVC_SAO_CTRL1, data32);
	READ_WRITE_DATA16(pbi, HEVC_SAO_CTRL1, 0x1, 0, 2);
	...
	//data32 |= 0x670;	// Big-Endian per 64-bit
	//data32 |= 0x880;	// Big-Endian per 64-bit
	READ_WRITE_DATA16(pbi, HEVC_SAO_CTRL1_DBE1, 0x880, 0, 12);
	//data32 &= (~0x3);
	//data32 |= 0x1; // [1]:dw_disable [0]:cm_disable
	//WRITE_VREG(HEVC_SAO_CTRL1_DBE1, data32);
	READ_WRITE_DATA16(pbi, HEVC_SAO_CTRL1_DBE1, 0x1, 0, 2);
	...
	//WRITE_VREG(HEVCD_IPP_AXIIF_CONFIG, data32);
	READ_WRITE_DATA16(pbi, HEVCD_IPP_AXIIF_CONFIG, 0x8, 0, 4);
	...
	//data32 |= 0x8;	// Big-Endian per 64-bit
	//WRITE_VREG(HEVCD_IPP_AXIIF_CONFIG_DBE1, data32);
	READ_WRITE_DATA16(pbi, HEVCD_IPP_AXIIF_CONFIG_DBE1, 0x8, 0, 4);
#endif
#else
	data32 = READ_VREG(HEVC_SAO_CTRL1);
	data32 &= (~(3 << 14));
	...
	if (get_cpu_major_id() >= AM_MESON_CPU_MAJOR_ID_G12A) { /* >= G12A dw write control */
		...
		if (dw_mode == 0)
			data = 1; /*enable first write*/
		else if (dw_mode & 0x10)
			data = 2; /*double write only*/
		else
			data = 3;
		//WRITE_VREG(HEVC_DBLK_CFGB, data);
		READ_WRITE_DATA16(pbi, HEVC_DBLK_CFGB, data, 8, 2);
	}
	...
	*/
	WRITE_BACK_32(pbi, HEVC_SAO_CTRL1, data32);
	...
	if (dw_mode & 0x10) {
		...
		if ((dw_mode & 0xf) == 8 ||
			(dw_mode & 0xf) == 9) {
			data = 0xff;
			WRITE_BACK_8(pbi, HEVC_SAO_CTRL26, 0xf);
			...
			data = 0x33;
		//WRITE_VREG(HEVC_SAO_CTRL5, data32);
		READ_WRITE_DATA16(pbi, HEVC_SAO_CTRL5, data, 16, 8);
	}
	...
	* [31:13] reserved
	*/
	WRITE_BACK_32(pbi, HEVCD_IPP_AXIIF_CONFIG, data32);
#endif
}

static int32_t config_mc_buffer_fb(struct VP9Decoder_s *pbi, unsigned short bit_depth)
{
	int32_t i;
	...
	WRITE_BACK_8(pbi, HEVCD_MPP_ANC_CANVAS_ACCCONFIG_ADDR, (0 << 8) | (0<<1) | 1);
	for (i = 0; i < REFS_PER_FRAME; ++i) {
		struct PIC_BUFFER_CONFIG_s *pic_config = cur_pic_config->pic_refs[i];
		if (!pic_config)
			continue;
		WRITE_BACK_32(pbi, HEVCD_MPP_ANC_CANVAS_DATA_ADDR, (pic_config->mc_canvas_u_v<<16)|(pic_config->mc_canvas_u_v<<8)|pic_config->mc_canvas_y);
		if (debug & VP9_DEBUG_BUFMGR_MORE)
		printk("refid %x mc_canvas_u_v %x mc_canvas_y %x\n",
			i,pic_config->mc_canvas_u_v,pic_config->mc_canvas_y);

	}
	WRITE_BACK_16(pbi, HEVCD_MPP_ANC_CANVAS_ACCCONFIG_ADDR, 0, (16 << 8) | (0<<1) | 1);
	for (i = 0; i < REFS_PER_FRAME; ++i) {
		struct PIC_BUFFER_CONFIG_s *pic_config = cur_pic_config->pic_refs[i];
		if (!pic_config)
			continue;
	...
	WRITE_BACK_8(pbi, VP9D_MPP_REFINFO_TBL_ACCCONFIG, 0x1 << 2); // auto_inc start index:0 field:0
	for (i=0; i<REFS_PER_FRAME; i++) // index 0:last 1:golden 2:altref
	{
		int32_t ref_pic_body_size;
		struct PIC_BUFFER_CONFIG_s *pic_config = cur_pic_config->pic_refs[i];

		WRITE_BACK_32(pbi, VP9D_MPP_REFINFO_DATA, pic_config->y_crop_width);
		WRITE_BACK_32(pbi, VP9D_MPP_REFINFO_DATA, pic_config->y_crop_height);
		vp9_print(pbi, VP9_DEBUG_DUAL_CORE, "refid %x y_crop_width 0x%x y_crop_height 0x%x\n", i,pic_config->y_crop_width,pic_config->y_crop_height);

		if (pic_config->y_crop_width != cur_pic_config->y_crop_width ||
		pic_config->y_crop_height != cur_pic_config->y_crop_height) {
		scale_enable |= (1<<i);
		}
		ref_pic_body_size = compute_losless_comp_body_size(pic_config->y_crop_width, pic_config->y_crop_height, (bit_depth == VPX_BITS_10));

		//printk("%d: ref width/height(%d,%d), cur width/height(%d,%d) ref_pic_body_size 0x%x\n",
		//  i, pic_config->y_crop_width, pic_config->y_crop_height,
		//  cur_pic_config->y_crop_width, cur_pic_config->y_crop_height,
		//  ref_pic_body_size);
		WRITE_BACK_32(pbi, VP9D_MPP_REFINFO_DATA, (pic_config->y_crop_width<<14)/cur_pic_config->y_crop_width);
		WRITE_BACK_32(pbi, VP9D_MPP_REFINFO_DATA, (pic_config->y_crop_height<<14)/cur_pic_config->y_crop_height);
		if (pbi->mmu_enable) {
			WRITE_BACK_8(pbi, VP9D_MPP_REFINFO_DATA, 0);
		} else {
			WRITE_BACK_32(pbi, VP9D_MPP_REFINFO_DATA, ref_pic_body_size >> 5);
		}
	}
	WRITE_BACK_8(pbi, VP9D_MPP_REF_SCALE_ENBL, scale_enable);

	return 0;
}

static int32_t config_pic_size_fb(struct VP9Decoder_s *pbi, unsigned short bit_depth)
{
	uint32_t data32;
	...
	vp9_print(pbi, VP9_DEBUG_DUAL_CORE, "    #### config_pic_size #### \n");
...
//    losless_comp_body_size_dw = compute_losless_comp_body_size_dw(frame_width, frame_height, (bit_depth == VPX_BITS_10));
//#endif

	//printk("%s: width %d height %d depth %d head_size 0x%x body_size 0x%x\r\n",
...
//pyx check done
	...
	READ_WRITE_DATA16(pbi, HEVC_SAO_CTRL5, data32, 9, 1);

	if (pbi->mmu_enable) {
		WRITE_BACK_8(pbi, HEVCD_MPP_DECOMP_CTL1,(0x1<< 4)); // bit[4] : paged_mem_mode
	} else {
		if (bit_depth == VPX_BITS_10) {
		WRITE_BACK_8(pbi, HEVCD_MPP_DECOMP_CTL1, (0<<3)); // bit[3] smem mdoe
		}
		else {
		WRITE_BACK_8(pbi, HEVCD_MPP_DECOMP_CTL1, (1<<3)); // bit[3] smem mdoe
		}
	}
	...
	WRITE_BACK_32(pbi, HEVC_CM_BODY_LENGTH,losless_comp_body_size);
	WRITE_BACK_32(pbi, HEVC_CM_HEADER_OFFSET,losless_comp_body_size);
	WRITE_BACK_32(pbi, HEVC_CM_HEADER_LENGTH,losless_comp_header_size);
	if (pbi->dw_mmu_enable) {
		WRITE_BACK_32(pbi, HEVC_CM_BODY_LENGTH2,losless_comp_body_size_dw);
		WRITE_BACK_32(pbi, HEVC_CM_HEADER_OFFSET2,losless_comp_body_size_dw);
		WRITE_BACK_32(pbi, HEVC_CM_HEADER_LENGTH2,losless_comp_header_size_dw);
	}
	if (cur_pic_config->double_write_mode & 0x10) {
		WRITE_BACK_32(pbi, HEVCD_MPP_DECOMP_CTL1, 0x1 << 31);
	}
#else
	WRITE_BACK_32(pbi, HEVCD_MPP_DECOMP_CTL1,0x1 << 31);
#endif
...
//pyx write h265, need check again
#define HEVC_BE_DECODE_DATA 0xa0

void BackEnd_StartDecoding(struct VP9Decoder_s *pbi)
{
	...
	vp9_print(pbi, VP9_DEBUG_DUAL_CORE, "Start BackEnd Decoding %d (wr pos %d, rd pos %d)\n",
	...
	if (pbi->mmu_enable) {
		// to do: !bit_depth_10  and page need/2 ?
		cur_mmu_4k_number = vp9_mmu_page_num(pbi, pic->y_crop_width, pic->y_crop_height/2 + 64 + 8, buf_alloc_depth == 10);
		if (cur_mmu_4k_number < 0)
				return;
		ret = decoder_mmu_box_alloc_idx(
			pbi->mmu_box,
	...
	print_loopbufs_ptr(pbi, "bk", &pbi->bk);
			...
			//print_hevc_b_data_path_monitor(pbi->backend_decoded_count-1); //pyx delete
			;
			}
#endif
#endif
...
static void vp9_init_decoder_hw_fb(struct VP9Decoder_s *pbi, int32_t decode_pic_begin, int32_t decode_pic_num, int first_flag, int front_flag, int back_flag)
{
...
#endif
	//printk("[test.c] Entering vp9_init_decoder_hw\n");
#if 0
	//printk("[test.c] Test Parser Register Read/Write\n");
	data32 = READ_VREG(P_HEVC_PARSER_VERSION);
	if (data32 != 0x00010001) { print_scratch_error(25); return; }
	WRITE_VREG(P_HEVC_PARSER_VERSION, 0x5a5a55aa);
	data32 = READ_VREG(P_HEVC_PARSER_VERSION);
	if (data32 != 0x5a5a55aa) { print_scratch_error(26); return; }

	// test Parser Reset
	WRITE_VREG(P_DOS_SW_RESET1, (1<<3)); // reset_whole parser
	WRITE_VREG(P_DOS_SW_RESET1, 0); // reset_whole parser
	data32 = READ_VREG(P_HEVC_PARSER_VERSION);
	...
	data32 = READ_VREG(P_HEVC_STREAM_CONTROL);
	data32 = data32 |
		(1 << 0) // stream_fetch_enable
		;
	WRITE_VREG(P_HEVC_STREAM_CONTROL, data32);

	data32 = READ_VREG(P_HEVC_SHIFT_STARTCODE);
	if (data32 != 0x00000100) { print_scratch_error(29); return; }
	data32 = READ_VREG(P_HEVC_SHIFT_EMULATECODE);
	if (data32 != 0x00000300) { print_scratch_error(30); return; }
	WRITE_VREG(P_HEVC_SHIFT_STARTCODE, 0x12345678);
	WRITE_VREG(P_HEVC_SHIFT_EMULATECODE, 0x9abcdef0);
	data32 = READ_VREG(P_HEVC_SHIFT_STARTCODE);
...
//             (1 << 0)     // fb vp9 en
//             ;
//    WRITE_VREG(P_HEVC_ASSIST_FB_CTL, data32);
//
//    data32 = (1 << 0);     // fb wr enable
//    WRITE_VREG(P_HEVC_ASSIST_FB_W_CTL, data32);
//
//    data32 = READ_VREG(P_HEVC_ASSIST_HED_FB_W_CTL);
//    data32 = data32 |
//             (1 << 0)    // hed_fb_wr_en
//             ;
//    WRITE_VREG(P_HEVC_ASSIST_HED_FB_W_CTL, data32);
//
//
//
//
//#ifdef VP9_10B_HED_FB
//    printk("[test.c] Init VP9_10B_HED_FB\n");
//    data32 = READ_VREG(P_HEVC_ASSIST_HED_FB_W_CTL);
//    data32 = data32 |
//             (1 << 0)    // hed_fb_wr_en
//             ;
...
//    printk("[test.c] Init VP9_10B_HED_SAME_FB\n");
//    data32 = READ_VREG(P_HEVC_ASSIST_HED_FB_R_CTL);
//    data32 = data32 |
//             (1 << 0)    // hed_fb_rd_en
//             ;
//    WRITE_VREG(P_HEVC_ASSIST_HED_FB_R_CTL, data32);
//#endif
//
//#endif
...
static void init_pic_list_hw_fb(struct VP9Decoder_s *pbi)
{
	...
	//WRITE_VREG(P_HEVCD_MPP_ANC2AXI_TBL_CONF_ADDR, 0x0);
	WRITE_BACK_8(pbi, HEVCD_MPP_ANC2AXI_TBL_CONF_ADDR, (0x1 << 1) | (0x1 << 2));
	...
	for (i=0; i<FRAME_BUFFERS; i++) {
		...
		if (pic_config->index >= 0) {
			if (pbi->mmu_enable && ((pic_config->double_write_mode & 0x10) == 0)) {
				WRITE_BACK_32(pbi, HEVCD_MPP_ANC2AXI_TBL_DATA, pic_config->header_adr>>5);
			} else {
				WRITE_BACK_32(pbi, HEVCD_MPP_ANC2AXI_TBL_DATA, pic_config->dw_y_adr>>5);
			}
			...
			WRITE_BACK_32(pbi, HEVCD_MPP_ANC2AXI_TBL_DATA, pic_config->mc_u_v_adr>>5);
#else
			if (get_double_write_mode(pbi) & 0x10) {
					WRITE_BACK_32(pbi, HEVCD_MPP_ANC2AXI_TBL_DATA, pic_config->dw_u_v_adr >> 5);
			}
#endif
		}
	}
	WRITE_BACK_8(pbi, HEVCD_MPP_ANC2AXI_TBL_CONF_ADDR, 0x1);
...
void print_hevc_b_data_path_monitor(int frame_count)
{
	...
	if (path_transfer_count == 0) path_wait_ratio = 0;
		...
		pr_info("[P%d HEVC CORE0 PATH] WAITING Ratio : %d",
		frame_count,
		path_wait_ratio);
	...
	if (path_transfer_count == 0) path_wait_ratio = 0;
		...
		pr_info("[P%d HEVC CORE1 PATH] WAITING Ratio : %d",
		frame_count,
		path_wait_ratio);
...
static void vp9_config_work_space_hw_fb(struct VP9Decoder_s *pbi, int front_flag, int back_flag)
{
	struct BuffInfo_s* buf_spec = pbi->work_space_buf;
	struct VP9_Common_s *const cm = &pbi->common;
	uint32_t data32;
	...
	if (cm->width == 0) {
		...
		losless_comp_body_size = compute_losless_comp_body_size(pbi->init_pic_w, pbi->init_pic_h, buf_alloc_depth == 10);
		losless_comp_header_size_dw = compute_losless_comp_header_size(pbi->init_pic_w, pbi->init_pic_h);
		losless_comp_body_size_dw = compute_losless_comp_body_size(pbi->init_pic_w, pbi->init_pic_h, buf_alloc_depth == 10);
	}
...
//--rb--
	...
	*/
	...
	if (pbi->mmu_enable) {
		...
		//WRITE_VREG(P_HEVCD_MPP_DECOMP_CTL2,0);
	} else {
		// if (cur_pic_config->bit_depth == VPX_BITS_10) WRITE_VREG(P_HEVCD_MPP_DECOMP_CTL1, (0<<3)); // bit[3] smem mdoe
...
void vp9_hw_init(struct VP9Decoder_s *pbi, int first_flag, int front_flag, int back_flag)
{
//pyx checked decode_pic_begin decode_pic_num need not use
	//int  decode_pic_begin = picParam[2];
	//int decode_pic_num = picParam[3];
...
//Start JT
	...
	if ((data32 != 0x00000100) && (data32 != 0x00000001)) { print_scratch_error(29); return; }
...
#endif
	WRITE_VREG(HEVC_SHIFT_STARTCODE, 0x00000001); // VP9 use 4 Bytes Startcode
...
// End JT
	...
	//WRITE_VREG(HEVC_ASSIST_AXIADDR_PREFIX, 0xa55a5aa5);
	//printk("HEVC_ASSIST_AXIADDR_PREFIX  : 0x%x\n", READ_VREG(HEVC_ASSIST_AXIADDR_PREFIX));
...
static int init_fb_bufstate(struct VP9Decoder_s *pbi)
{
	...
	size += pbi->fb_buf_lmem0.buf_size;
	...
	pbi->fb_buf_mpred_imp0.buf_size = IFBUF_MPRED_IMP0_SIZE * pbi->fb_ifbuf_num;
	size += pbi->fb_buf_mpred_imp0.buf_size;
	ret = decoder_bmmu_box_alloc_buf_phy(pbi->bmmu_box,
	...
	pbi->fb_buf_mpred_imp1.buf_size = IFBUF_MPRED_IMP1_SIZE * pbi->fb_ifbuf_num;
	size += pbi->fb_buf_mpred_imp1.buf_size;
	ret = decoder_bmmu_box_alloc_buf_phy(pbi->bmmu_box,
	...
	vp9_print(pbi, VP9_DEBUG_DUAL_CORE, "%s workspace (%x %x) size = %x\n", __func__, pbi->fb_buf_mmu0.buf_start, pbi->fb_buf_mpred_imp1.buf_end, pbi->fb_buf_mpred_imp1.buf_end-pbi->fb_buf_mmu0.buf_start);
	vp9_print(pbi, VP9_DEBUG_DUAL_CORE, "fb_buf_mmu0.buf_start        :%x\n"  , pbi->fb_buf_mmu0.buf_start         );
	vp9_print(pbi, VP9_DEBUG_DUAL_CORE, "fb_buf_mmu1.buf_start        :%x\n"  , pbi->fb_buf_mmu1.buf_start         );
	vp9_print(pbi, VP9_DEBUG_DUAL_CORE, "fb_buf_lmem0.buf_start       :%x\n"  , pbi->fb_buf_lmem0.buf_start            );
	vp9_print(pbi, VP9_DEBUG_DUAL_CORE, "fb_buf_mpred_imp0.buf_start  :%x\n"  , pbi->fb_buf_mpred_imp0.buf_start            );
	vp9_print(pbi, VP9_DEBUG_DUAL_CORE, "fb_buf_mpred_imp1.buf_start  :%x\n"  , pbi->fb_buf_mpred_imp1.buf_start            );
	vp9_print(pbi, VP9_DEBUG_DUAL_CORE, "total size                   :%x\n"  , size);
	...
	pbi->fr.lmem0_ptr = pbi->fb_buf_lmem0.buf_start;
	pbi->bk.lmem0_ptr = pbi->fb_buf_lmem0.buf_start;
...
{
	//pyx modify need check
	int i;
	for (i = BMMU_IFBUF_LMEM0_ID; i <= BMMU_IFBUFF_MPRED_IMP1_ID; i++) {
		decoder_bmmu_box_free_idx(pbi->bmmu_box, i);
	}

	if (pbi->fb_buf_mmu0_addr) {
		dma_free_coherent(amports_get_dma_device(),
				pbi->fb_buf_mmu0.buf_size, pbi->fb_buf_mmu0_addr,
					pbi->fb_buf_mmu0.buf_start);
		pbi->fb_buf_mmu0_addr = NULL;
	}
	if (pbi->fb_buf_mmu1_addr) {
		dma_free_coherent(amports_get_dma_device(),
				pbi->fb_buf_mmu1.buf_size, pbi->fb_buf_mmu1_addr,
					pbi->fb_buf_mmu1.buf_start);
		pbi->fb_buf_mmu1_addr = NULL;
	}
	if (pbi->mmu_box_fb) {
		decoder_mmu_box_free(pbi->mmu_box_fb);
		pbi->mmu_box_fb = NULL;
	}
...
//pyx  no show frame how to do
static void read_bufstate_front(struct VP9Decoder_s *pbi)
{
	pbi->fr.mmu0_ptr = READ_VREG(HEVC_ASSIST_FB_MMU_MAP_ADDR0);
	pbi->fr.mmu1_ptr = READ_VREG(HEVC_ASSIST_FB_MMU_MAP_ADDR1);
	WRITE_VREG(HEVC_ASSIST_RING_F_INDEX, 4);
	pbi->fr.lmem0_ptr = READ_VREG(HEVC_ASSIST_RING_F_WPTR);
	WRITE_VREG(HEVC_ASSIST_RING_F_INDEX, 2);
...
static void print_bufstate_front(struct VP9Decoder_s *pbi)
{
	vp9_print(pbi, VP9_DEBUG_DUAL_CORE, "mmu0_ptr: 0x%x\n", pbi->fr.mmu0_ptr);
	vp9_print(pbi, VP9_DEBUG_DUAL_CORE, "mmu1_ptr: 0x%x\n", pbi->fr.mmu1_ptr);
	vp9_print(pbi, VP9_DEBUG_DUAL_CORE, "lmem0buf fwptr : 0x%x\n", pbi->fr.lmem0_ptr);
	vp9_print(pbi, VP9_DEBUG_DUAL_CORE, "mpredimp0 fwptr : 0x%x\n", pbi->fr.mpred_imp0_ptr);
	vp9_print(pbi, VP9_DEBUG_DUAL_CORE, "mpredimp1 fwptr : 0x%x\n", pbi->fr.mpred_imp1_ptr);
}

static int front_decpic_done_update(struct VP9Decoder_s *pbi)
{
	struct VP9_Common_s *cm = &pbi->common;
	int i;
	//WRITE_VREG(HEVC_DECODE_SIZE, 0xffffffff);
	vp9_print(pbi, VP9_DEBUG_DUAL_CORE, "========= HEVC_DECPIC_DATA_DONE == \n");

	if (pbi->front_back_mode == 1) {
		read_bufstate_front(pbi);
		print_bufstate_front(pbi);

		WRITE_VREG(HEVC_ASSIST_FB_PIC_CLR, 1);

		WRITE_VREG(HEVC_DEC_STATUS_REG, HEVC_DEC_IDLE);
		pbi->frontend_decoded_count++;
		pbi->next_be_decode_pic[pbi->fb_wr_pos] = &cm->cur_frame->buf;
		mutex_lock(&pbi->fb_mutex);
		pbi->fb_wr_pos++;
		if (pbi->fb_wr_pos >= pbi->fb_ifbuf_num)
			pbi->fb_wr_pos = 0;

		if (pbi->fb_wr_pos == pbi->fb_rd_pos) {
			pbi->wait_working_buf = 1;
			vp9_print(pbi, VP9_DEBUG_DUAL_CORE, "fb_wr_pos %d fb_rd_pos %d, set wait_working_buf = 1\n", pbi->fb_wr_pos, pbi->fb_rd_pos);
		} else {
			vp9_print(pbi, VP9_DEBUG_DUAL_CORE, "fb_wr_pos %d fb_rd_pos %d\n", pbi->fb_wr_pos, pbi->fb_rd_pos);
		}
...
#ifdef NEW_FRONT_BACK_CODE
static void init_buff_spec_fb(struct VP9Decoder_s *pbi, struct BuffInfo_s* buf_spec)
{
//pyx check
		//buf_spec->ipp.buf_start = buf_spec->start_adr;
		buf_spec->ipp0.buf_start = buf_spec->start_adr;
		buf_spec->ipp1.buf_start = buf_spec->ipp0.buf_start + buf_spec->ipp0.buf_size;
		buf_spec->sao_abv.buf_start = buf_spec->ipp1.buf_start + buf_spec->ipp1.buf_size;
		...
		buf_spec->seg_map.buf_start = buf_spec->dblk_data.buf_start + buf_spec->dblk_data.buf_size;
		if (pbi == NULL || pbi->mmu_enable) {
			buf_spec->mmu_vbh.buf_start  = buf_spec->seg_map.buf_start + buf_spec->seg_map.buf_size;
			buf_spec->cm_header.buf_start  = buf_spec->mmu_vbh.buf_start + buf_spec->mmu_vbh.buf_size;
#ifdef VP9_10B_MMU_DW
			if (pbi == NULL || pbi->dw_mmu_enable) {
				buf_spec->mmu_vbh_dw.buf_start	= buf_spec->cm_header.buf_start + buf_spec->cm_header.buf_size;
				buf_spec->cm_header_dw.buf_start = buf_spec->mmu_vbh_dw.buf_start + buf_spec->mmu_vbh_dw.buf_size;
				buf_spec->mpred_above.buf_start = buf_spec->cm_header_dw.buf_start + buf_spec->cm_header_dw.buf_size;
			} else {
				buf_spec->mpred_above.buf_start = buf_spec->cm_header.buf_start + buf_spec->cm_header.buf_size;
			}
#else
			buf_spec->mpred_above.buf_start = buf_spec->cm_header.buf_start + buf_spec->cm_header.buf_size;
#endif
		} else {
			buf_spec->mpred_above.buf_start = buf_spec->seg_map.buf_start + buf_spec->seg_map.buf_size;
		}
		...
		if (debug) {
			printk("ipp0.buf_start			   :%x\n"  , buf_spec->ipp0.buf_start		  );
			printk("ipp1.buf_start			   :%x\n"  , buf_spec->ipp1.buf_start		  );
			...
			printk("lmem.buf_start		  :%x\n"  , buf_spec->lmem.buf_start	   );
			printk("frame_header_lmem.buf_start 			:%x\n"	, buf_spec->frame_header_lmem.buf_start 		   );
...
#ifdef NEW_FB_CODE
	if (pbi->front_back_mode && pbi->mmu_box_1) {
		decoder_mmu_box_free(pbi->mmu_box_1);
		pbi->mmu_box_1 = NULL;
	}
#endif
...
#ifdef NEW_FB_CODE
	if (pbi->front_back_mode && pbi->mmu_box_dw_1) {
		decoder_mmu_box_free(pbi->mmu_box_dw_1);
		pbi->mmu_box_dw_1 = NULL;
	}
#endif

#ifdef NEW_FB_CODE
		if (pbi->front_back_mode)
			uninit_fb_bufstate(pbi);
#endif
				...
				struct PIC_BUFFER_CONFIG_s *pic_config)
{
...
#ifdef P010_ENABLE
		if (is_dw_p010(pbi)) {  //double size mem for p010 mode
			buf_size += ((mc_buffer_size_u_v_h << 16) * 3);
		}
#endif
	}
	...
	i = pic_config->index;
#ifdef MV_USE_FIXED_BUF
	if ((pbi->work_space_buf->mpred_mv.buf_start + ((i + 1) * mv_size)) <= mpred_mv_end) {
#endif
		if (buf_size > 0) {
			ret = decoder_bmmu_box_alloc_buf_phy(pbi->bmmu_box,
					VF_BUFFER_IDX(i),
			...
			if (ret < 0) {
				pr_info("decoder_bmmu_box_alloc_buf_phy idx %d size %d fail\n",
			...
			if (pic_config->cma_alloc_addr)
				...
				pr_info("decoder_bmmu_box_alloc_buf_phy idx %d size %d return null\n",
					VF_BUFFER_IDX(i), buf_size);
				return -1;
			}
		}
		{
			dw_uv_size = mc_buffer_size_u_v_h << (16 + is_dw_p010(pbi));
			...
			if (tw_mode) {
				...
				pic_config->tw_u_v_adr = pic_config->tw_y_adr + (mc_buffer_size_u_v_h << (16 + is_tw_p010(pbi) + 1));
			}
#endif
			...
			if (debug) {
				pr_info("%s index %d BUF_index %d ",
					__func__, pic_config->index, pic_config->BUF_index);
				pr_info("comp_body_size %x comp_buf_size %x ",
					pic_config->comp_body_size, pic_config->buf_size);
				pr_info("mpred_mv_wr_start_adr %ld\n",
					pic_config->mpred_mv_wr_start_addr);
				pr_info("dw_y_adr %d, pic_config->dw_u_v_adr =%d\n",
					pic_config->dw_y_adr, pic_config->dw_u_v_adr);
...
{
//pyx confirm need new function like simulation code?
	int i;
	struct VP9_Common_s *cm = &pbi->common;
	struct PIC_BUFFER_CONFIG_s *pic_config;
	u32 header_size;
	...
	for (i = 0; i < pbi->used_buf_num; i++) {
		...
		if (vdec->parallel_dec == 1) {
			...
			pic_config->tw_y_canvas_index = -1;
			pic_config->tw-uv_canvas_index = -1;
#endif
		}
...
#ifdef OW_TRIPLE_WRITE
		pic_config->triple_write_mode = get_triple_write_mode(pbi);
#endif
		if (!pbi->is_used_v4l) {
...
static void dump_pic_list(struct VP9Decoder_s *pbi)
{
	...
	for (i = 0; i < FRAME_BUFFERS; i++) {
		pic_config = &cm->buffer_pool->frame_bufs[i].buf;
		vp9_print(pbi, 0,
			"Buf(%d) index %d mv_buf_index %d ref_count %d vf_ref %d dec_idx %d slice_type %d w/h %d/%d adr: %x, backend_ref %d, back_done_mark %d\n",
			i,
			pic_config->index,
#ifndef MV_USE_FIXED_BUF
	...
	if (get_double_write_mode(pbi) & 0x10) {
		if (is_dw_p010(pbi)) {
			...
			WRITE_VREG(HEVCD_MPP_DECOMP_CTL1,
				(0x1 << 31) | (1 << 24) | (((pbi->endian >> 12) & 0xff) << 16));
		} else {
...
#ifdef NEW_FRONT_BACK_CODE
	if (pbi->front_back_mode == 1) {
		WRITE_VREG(VP9D_MPP_REFINFO_DATA_DBE1, (pic_config->y_crop_width<<14)/cur_pic_config->y_crop_width);
		WRITE_VREG(VP9D_MPP_REFINFO_DATA_DBE1, (pic_config->y_crop_height<<14)/cur_pic_config->y_crop_height);
	}
#endif
	if (pbi->mmu_enable) {
...
static void clear_mpred_hw(struct VP9Decoder_s *pbi)
{
	...
	data32 &=  (~(1 << 6));
	if (get_cpu_major_id() >= AM_MESON_CPU_MAJOR_ID_S5)
		WRITE_VREG(HEVC_MPRED_CTRL3,0x24122412); // use short burst length for costdown chip
	WRITE_VREG(HEVC_MPRED_CTRL4, data32);
}
...
static void config_sao_hw(struct VP9Decoder_s *pbi, union param_u *params)
{
...
#endif
		data32 = mc_buffer_size_u_v_h << (16 + is_dw_p010(pbi) + 1);
		WRITE_VREG(HEVC_SAO_Y_LENGTH, data32);
		data32 = mc_buffer_size_u_v_h << (16 + is_dw_p010(pbi));
		WRITE_VREG(HEVC_SAO_C_LENGTH, data32);

		if (tw_mode) {
			data32 = mc_buffer_size_u_v_h << (16 + is_tw_p010(pbi) + 1);
			WRITE_VREG(HEVC_SAO_Y_LENGTH3, data32);
			data32 = mc_buffer_size_u_v_h << (16 + is_tw_p010(pbi));
			WRITE_VREG(HEVC_SAO_C_LENGTH3, data32);

			if (debug & VP9_DEBUG_REG) {
				vp9_print(pbi, 0, "triple write SAO_Y_ADDR3 %x, SAO_Y_LENGTH3 %x, SAO_C_ADDR3 %x, SAO_C_LENGTH3 %x\n",
	...
	*/
	...
	if (tw_mode) {
		...
		if (debug & VP9_DEBUG_REG) {
			vp9_print(pbi, VP9_DEBUG_REG,
				"%s, HEVC_SAO_CTRL3 0x%x HEVC_SAO_CTRL31 0x%x HEVC_SAO_CTRL32 0x%x\n",
...
void vp9_loop_filter_init(struct VP9Decoder_s *pbi)
{
	...
	for (i = 0; i < 32; i++) {
		...
		if (pbi->front_back_mode == 1)
			WRITE_VREG(HEVC_DBLK_CFG9_DBE1, thr); //pyx ffff
	}
...
void vp9_loop_filter_frame_init_fb(struct VP9Decoder_s *pbi, struct segmentation *seg,
	...
	/*Write to register*/
		for (i = 0; i < 32; i++) {
			...
			thr = (thr << 16) | ((lfi->lfthr[i * 2].lim & 0x3f) << 8)
				| (lfi->lfthr[i * 2].mblim & 0xff);
			WRITE_BACK_32(pbi, HEVC_DBLK_CFG9, thr);
		}
	}
	...
	for (i = 0; i < 16; i++) {
		...
		if (!default_filt_lvl)
			level = 0;
		WRITE_BACK_32(pbi, HEVC_DBLK_CFGA, level);
	}
}
/* VP9_LPF_LVL_UPDATE */
#endif
...
static void  config_mcrcc_axi_hw_fb(struct VP9Decoder_s *pbi)
{
	u32 curr_picnum = -1;
	...
	WRITE_BACK_8(pbi, HEVCD_MCRCC_CTL1, 0x2); /* reset mcrcc */
	...
	if (!is_inter) { /* I-PIC */
		/* remove reset -- disables clock */
		WRITE_BACK_8(pbi, HEVCD_MCRCC_CTL1, 0x0);
		return;
	}
	...
	WRITE_BACK_8(pbi, HEVCD_MPP_ANC_CANVAS_ACCCONFIG_ADDR, (0 << 8) | (1<<1) | 0);
	...
	if (mcrcc_cache_alg_flag ==
		...
		rdata32 = rdata32 & 0xffff;
		rdata32 = rdata32 | (rdata32 << 16);
		WRITE_BACK_32(pbi, HEVCD_MCRCC_CTL2, rdata32);

		/* 2nd-lowest delta_picnum */
		rdata32 = refcanvas_array[1];
		rdata32 = rdata32 & 0xffff;
		rdata32 = rdata32 | (rdata32 << 16);
		WRITE_BACK_32(pbi, HEVCD_MCRCC_CTL3, rdata32);
	} else {
		/* previous version -- LAST/GOLDEN ALWAYS -- before 09/13/2017*/
		WRITE_BACK_8(pbi, HEVCD_MPP_ANC_CANVAS_ACCCONFIG_ADDR,
			(0 << 8) | (1<<1) | 0);
		//rdata32 = READ_VREG(HEVCD_MPP_ANC_CANVAS_DATA_ADDR);
		//rdata32 = rdata32 & 0xffff;
		//rdata32 = rdata32 | (rdata32 << 16);
		READ_INS_WRITE(pbi, HEVCD_MPP_ANC_CANVAS_DATA_ADDR, HEVCD_MCRCC_CTL2, 0, 16, 16);
		...
		//rdata32 = rdata32 | (rdata32 << 16);
		//WRITE_VREG(HEVCD_MCRCC_CTL3, rdata32);
		READ_INS_WRITE(pbi, HEVCD_MPP_ANC_CANVAS_DATA_ADDR, HEVCD_MCRCC_CTL3, 0, 16, 16);

	}

	WRITE_BACK_16(pbi, HEVCD_MCRCC_CTL1, 0, 0xff0); /* enable mcrcc progressive-mode */
...
static int alloc_lf_buf(struct VP9Decoder_s *pbi)
{
	...
	if (pbi->lfi == NULL || pbi->lf == NULL || pbi->seg_4lf == NULL) {
		free_lf_buf(pbi);
		vp9_print(pbi, VP9_DEBUG_DUAL_CORE, "[test.c] vp9_loop_filter init malloc error!!!\n");
		return -1;
	}
	return 0;
}
...
#ifdef NEW_FB_CODE
		if (pbi->front_back_mode) {
			if (pbi->frame_mmu_map_addr_1) {
				if (pbi->frame_mmu_map_phy_addr_1)
					dma_free_coherent(amports_get_dma_device(),
						mmu_map_size,
						pbi->frame_mmu_map_addr_1,
						pbi->frame_mmu_map_phy_addr_1);
				pbi->frame_mmu_map_addr_1 = NULL;
			}
		}
#endif
	}
...
#ifdef NEW_FB_CODE
		if (pbi->front_back_mode) {
			if (pbi->frame_mmu_dw_map_addr_1) {
			if (pbi->frame_mmu_dw_map_phy_addr_1)
				dma_free_coherent(amports_get_dma_device(),
					dw_mmu_map_size,
					pbi->frame_mmu_dw_map_addr_1,
					pbi->frame_mmu_dw_map_phy_addr_1);
			pbi->frame_mmu_dw_map_addr_1 = NULL;
		}
		}
#endif
	}
...
static int vp9_local_init(struct VP9Decoder_s *pbi)
{
	...
	if (force_bufspec) {
		...
		if (pbi->front_back_mode == 1 || pbi->front_back_mode == 3) {
			memcpy(cur_buf_info, &amvvp9_workbuff_spec[6],
					sizeof(struct BuffInfo_s));
		} else
#endif
...
#ifdef NEW_FRONT_BACK_CODE
	if (pbi->front_back_mode == 1 || pbi->front_back_mode == 3)
		init_buff_spec_fb(pbi, cur_buf_info);
	else
#endif
	...
	*/
	...
	if (pbi->mmu_enable) {
		...
		if (pbi->front_back_mode) {
			pbi->frame_mmu_map_addr_1 =
				dma_alloc_coherent(amports_get_dma_device(),
				mmu_map_size,
				&pbi->frame_mmu_map_phy_addr_1, GFP_KERNEL);
			if (pbi->frame_mmu_map_addr_1 == NULL) {
				pr_err("%s: failed to alloc count_buffer\n", __func__);
				return IRQ_HANDLED;
			}
			memset(pbi->frame_mmu_map_addr_1, 0, mmu_map_size);
		}
#endif
	}
#ifdef VP9_10B_MMU_DW
	if (pbi->dw_mmu_enable) {
		...
		memset(pbi->frame_mmu_dw_map_addr, 0, dw_mmu_map_size);
#ifdef NEW_FB_CODE
		if (pbi->front_back_mode) {
			pbi->frame_mmu_dw_map_addr_1 =
			dma_alloc_coherent(amports_get_dma_device(),
				dw_mmu_map_size,
				&pbi->frame_mmu_dw_map_phy_addr_1, GFP_KERNEL);
			if (pbi->frame_mmu_dw_map_addr_1 == NULL) {
				pr_err("%s: failed to alloc count_buffer mmu dw\n", __func__);
				return -1;
			}
			memset(pbi->frame_mmu_dw_map_addr_1, 0, dw_mmu_map_size);
		}
#endif
	}
#endif
	...
	struct PIC_BUFFER_CONFIG_s *pic_config)
{
	...
	if	(pic_config->double_write_mode) {
		...
		pic_config->canvas_config[0].bit_depth = is_dw_p010(pbi);
		...
		pic_config->canvas_config[1].bit_depth = is_dw_p010(pbi);
#endif
	}
	...
	if (pic_config->triple_write_mode) {
		...
		pic_config->tw_canvas_config[0].bit_depth  = is_tw_p010(pbi);
		...
		pic_config->tw_canvas_config[1].bit_depth  = is_tw_p010(pbi);
	}
#endif
}
...
static struct vframe_s *vvp9_vf_peek(void *op_arg)
{
...
#ifdef NEW_FB_CODE
	{
		...
		if (pbi->front_back_mode) {
			if (kfifo_peek(&pbi->display_q, (void *)&vf_tmp) && vf_tmp) {
				u32 index;
				struct VP9_Common_s *cm = &pbi->common;
				struct BufferPool_s *pool = cm->buffer_pool;
...
static struct vframe_s *vvp9_vf_get(void *op_arg)
{
...
#ifdef NEW_FB_CODE
		if (pbi->front_back_mode) {
			if (kfifo_peek(&pbi->display_q, &vf) && vf) {
				uint8_t index;
				struct VP9_Common_s *cm = &pbi->common;
				struct BufferPool_s *pool = cm->buffer_pool;
	...
	if (kfifo_get(&pbi->display_q, &vf)) {
		...
		if (index < pbi->used_buf_num ||
			...
			vf->frame_index = pbi->vf_get_count;
			pbi->vf_get_count++;
			if (debug & VP9_DEBUG_BUFMGR)
...
static void vvp9_vf_put(struct vframe_s *vf, void *op_arg)
{
	...
	if (index < pbi->used_buf_num) {
		...
		if (pbi->wait_buf)
			WRITE_VREG(pbi->ASSIST_MBOX0_IRQ_REG,
						0x1);
				...
				struct PIC_BUFFER_CONFIG_s *pic_config)
{
	...
	if (vf) {
		...
		if (!pic_config->double_write_mode && pic_config->triple_write_mode) {
			...
			vf->height = pic_config->y_crop_height /
				get_double_write_ratio(pic_config->triple_write_mode & 0xf);
			vp9_print(pbi, 0, "output triple write w %d, h %d bitdepth %s\n",
...
int continue_decoding(struct VP9Decoder_s *pbi)
{
	...
	if (ret < 0) {
		...
		if (pbi->front_back_mode) {
			int ii,j;
			for (ii = 0; ii < REFS_PER_FRAME; ii++)
				cur_pic_config->pic_refs[ii] = cm->frame_refs[ii].buf;
			mutex_lock(&pbi->fb_mutex);
			cur_pic_config->backend_ref = 1;
			for (j = 0; j < REFS_PER_FRAME; j++) {
				if (cur_pic_config->pic_refs[j])
				  cur_pic_config->pic_refs[j]->backend_ref++;
			}
			mutex_unlock(&pbi->fb_mutex);
		}
#endif
		...
		if (efficiency_mode == 0 &&
			...
			if (cur_pic_config->pic_refs[0])
				config_mc_buffer_fb(pbi, pbi->vp9_param.p.bit_depth);
			config_mcrcc_axi_hw_fb(pbi);
			config_sao_hw_fb(pbi);
			config_loop_filter_hw(pbi, &pbi->vp9_param);
		}
#endif
	...
	if ((get_cpu_major_id() >= AM_MESON_CPU_MAJOR_ID_S5) && !pbi->front_back_mode)	{
		// Disable interface which will reset history -- can be match with Dual result with RESET
			WRITE_VREG(HEVC_PARSER_IF_CONTROL,
					//	(1 << 8) | // sao_sw_pred_enable
					  (1 << 5) | // parser_sao_if_en
					  (1 << 2) | // parser_mpred_if_en
					  (0 << 0) // parser_scaler_if_en
					);
			WRITE_VREG(HEVC_PARSER_IF_CONTROL,
					//	(1 << 8) | // sao_sw_pred_enable
					  (1 << 5) | // parser_sao_if_en
		...
		*/
		...
		vp9_loop_filter_frame_init(pbi, pbi->seg_4lf,
			...
			//pbi->ins_offset = 0; //for next slice
			//copyToDDR_32bits(hevc->fr.sys_imem_ptr, instruction, ins_offset*4, 0);
			pbi->sys_imem_ptr += FB_IFBUF_SYS_IMEM_BLOCK_SIZE;
			pbi->sys_imem_ptr_v += FB_IFBUF_SYS_IMEM_BLOCK_SIZE;
#endif
...
#ifdef NEW_FB_CODE
irqreturn_t vp9_back_irq_cb(struct vdec_s *vdec, int irq)
{
	struct VP9Decoder_s *pbi =
	...
	if (pbi->dec_status_back == HEVC_BE_DECODE_DATA_DONE) {
		vdec_profile(hw_to_vdec(pbi), VDEC_PROFILE_DECODER_PIC_END, CORE_MASK_HEVC_BACK);
	}

	WRITE_VREG(pbi->backend_ASSIST_MBOX0_CLR_REG, 1);

	if (pbi->front_back_mode != 1) {
		vp9_print(pbi, VP9_DEBUG_DUAL_CORE,"[BE] %s\n", __func__);
		return IRQ_WAKE_THREAD;
	}

	/*BackEnd_Handle()*/
	if (READ_VREG(DEBUG_REG1_DBE)) {
		vp9_print(pbi, VP9_DEBUG_DUAL_CORE,"[BE] dbg%x: %x\n", READ_VREG(DEBUG_REG1_DBE),
...
irqreturn_t vp9_back_threaded_irq_cb(struct vdec_s *vdec, int irq)
{
	struct VP9Decoder_s *pbi =
		(struct VP9Decoder_s *)vdec->private;
	...
	if ((dec_status == HEVC_BE_DECODE_DATA_DONE)  || pbi->front_back_mode == 2 || pbi->front_back_mode == 3) {
		...
		if (pbi->front_back_mode == 1) {
			if (get_cpu_major_id() >= AM_MESON_CPU_MAJOR_ID_S5)
				vp9_print(pbi, VP9_DEBUG_DUAL_CORE, "pic %d HEVC_SAO_CRC 0x%x HEVC_SAO_CRC_DBE1 0x%x\n",
						pbi->backend_decoded_count, READ_VREG(HEVC_SAO_CRC), READ_VREG(HEVC_SAO_CRC_DBE1));
		}
...
static irqreturn_t vvp9_isr_thread_fn(int irq, void *data)
{
	...
	if ((dec_status == HEVC_NAL_DECODE_DONE) ||
		...
		if (efficiency_mode) {
			if (!wait_for_completion_timeout(&pbi->complete, msecs_to_jiffies(34)))
				vp9_print(pbi, 0, "!!!wait for completion timeout %d\n", __LINE__);
		}
	...
	if (dec_status == VP9_HEAD_PARSER_DONE) {
		//WRITE_VREG(0x3e3176, 0x6);
		//pr_err("VP9_ACP_CTRL = %d\n", READ_VREG(0x3e3176));
		if (debug & 1)
			dump_pic_list(pbi);
		if (pbi->m_ins_flag &&
...
#if 1
			//pyx can not free tail on front but need free on other place
			if (pbi->mmu_enable
#ifdef NEW_FB_CODE
				&& ((pbi->front_back_mode == 0) || (pbi->front_back_mode == 2))
#endif
				) {
				ATRACE_COUNTER(pbi->trace.decode_header_memory_time_name, TRACE_HEADER_MEMORY_START);
		...
		if (debug & VP9_DEBUG_SEND_PARAM_WITH_REG) {
...
#endif
			{
				/*pr_err("test rpm\n");
				for (i = 0; i < (RPM_END-RPM_BEGIN); i++) {
						pr_info("%04x ", pbi->rpm_ptr[i]);
						if (((i + 1) & 0xf) == 0)
							pr_info("\n");
					}
				pr_err("test rpm done\n");*/
				ATRACE_COUNTER(pbi->trace.decode_header_memory_time_name, TRACE_HEADER_RPM_START);
				if (efficiency_mode) {
					memcpy(pbi->vp9_param.l.data, pbi->rpm_ptr,
						(RPM_VALID_E - RPM_BEGIN) * sizeof(pbi->rpm_ptr[0]));
				} else {
	...
	if (efficiency_mode &&
		dec_status == VP9_HEAD_PARSER_DONE) {
		struct VP9_Common_s *const cm = &pbi->common;
		struct PIC_BUFFER_CONFIG_s *cur_pic_config
			= &cm->cur_frame->buf;
...
static irqreturn_t vvp9_isr(int irq, void *data)
{
	...
	if (dec_status == HEVC_DECPIC_DATA_DONE) {
		vdec_profile(hw_to_vdec(pbi), VDEC_PROFILE_DECODER_PIC_END, CORE_MASK_HEVC);
	}

	WRITE_VREG(pbi->ASSIST_MBOX0_CLR_REG, 1);

	if (dec_status == VP9_HEAD_PARSER_DONE) {
		vdec_profile(hw_to_vdec(pbi), VDEC_PROFILE_DECODER_HEADER_END, CORE_MASK_HEVC);
	...
	if (debug_tag & 0x10000) {
		...
		if ((udebug_pause_pos == (debug_tag & 0xffff)) &&
			...
			udebug_pause_val == READ_HREG(DEBUG_REG2)))
			pbi->ucode_pause_pos = udebug_pause_pos;
		if (test_dbg == READ_HREG(DEBUG_REG1)) {
			pbi->dec_result = DEC_RESULT_DONE;
			test_dbg = 0;
			vp9_print(pbi, 0,
				"ammvdec_vp9: dbg test next\n");
...
#ifdef NEW_FB_CODE
static void vvp9_put_timer_func_back(struct timer_list *timer)
{
	...
	if (
		...
		if (pbi->decode_timeout_count_back > 0)
			pbi->decode_timeout_count_back--;
		if (pbi->decode_timeout_count_back == 0)
			timeout_process_back(pbi);
	}
...
static void vvp9_put_timer_func(struct timer_list *timer)
{
	...
	if (debug & VP9_DEBUG_TRIG_SLICE_SEGMENT_PROC) {
		WRITE_VREG(pbi->ASSIST_MBOX0_IRQ_REG, 0x1);
...
static s32 vvp9_init(struct VP9Decoder_s *pbi)
{
	...
	if (pbi->front_back_mode == 1 || pbi->front_back_mode == 3) {
		...
		fw_back->len = get_firmware_data(VIDEO_DEC_VP9_BACK, fw_back->data);
		if (fw_back->len < 0) {
...
#ifdef NEW_FB_CODE
		if (pbi->front_back_mode) {
			pbi->timer_back.expires = jiffies + PUT_INTERVAL;
			pbi->fw_back = fw_back;
		}
#endif
		/*add_timer(&pbi->timer);
...
#ifdef NEW_FB_CODE
		if (pbi->front_back_mode) {
			INIT_WORK(&pbi->work_back, vp9_work_back);
		}
#endif
...
#ifdef NEW_FB_CODE
		if (pbi->front_back_mode && (pbi->stat & STAT_TIMER_BACK_ARM)) {
...
#ifdef NEW_FB_CODE
	if (pbi->front_back_mode) {
		cancel_work_sync(&pbi->work_back);
		vfree(pbi->fw_back);
		pbi->fw_back = NULL;
	}
#endif
...
static int vvp9_stop(struct VP9Decoder_s *pbi)
{
	...
	if (pbi->stat & STAT_ISR_REG) {
		...
		if (!pbi->m_ins_flag)
#endif
			WRITE_VREG(pbi->ASSIST_MBOX0_MASK, 0);
		vdec_free_irq(VDEC_IRQ_0, (void *)pbi);
		pbi->stat &= ~STAT_ISR_REG;
	}
...
static int amvdec_vp9_mmu_init(struct VP9Decoder_s *pbi)
{
	...
	if (pbi->mmu_enable && !pbi->is_used_v4l) {
		...
		if (pbi->front_back_mode) {
			...
			if (!pbi->mmu_box_1) {
				pr_err("vp9 alloc mmu box 1 failed!!\n");
				return -1;
			}
		}
#endif
	...
	if (pbi->dw_mmu_enable && !pbi->is_used_v4l) {
		...
		if (pbi->front_back_mode) {
			...
			if (!pbi->mmu_box_dw_1) {
				pr_err("vp9 alloc mmu dw box 1 failed!!\n");
			}
		}
#endif
	}
	...
	struct vdec_s *vdec)
{
	...
	if (pbi->vdec_back_cb)
		pbi->vdec_back_cb(hw_to_vdec(pbi), pbi->vdec_back_cb_arg, CORE_MASK_HEVC_BACK);
...
static void vp9_work_implement(struct VP9Decoder_s *pbi)
{
	...
	if (pbi->dec_result == DEC_RESULT_AGAIN) {
		vdec_profile(hw_to_vdec(pbi), VDEC_PROFILE_EVENT_AGAIN, CORE_MASK_HEVC);
...
static unsigned long check_input_data(struct vdec_s *vdec, unsigned long mask)
{
	struct VP9Decoder_s *pbi =
		(struct VP9Decoder_s *)vdec->private;
	//pr_err("check_input_data\n");
	if (pbi->front_back_mode == 1) {
		if (((pbi->fb_wr_pos != pbi->fb_rd_pos) || pbi->wait_working_buf))
			return mask;
		else {
			//pr_err("%x, wr %d rd %d, wait %d\n",
				//READ_VREG(HEVC_DEC_STATUS_DBE), pbi->fb_wr_pos, pbi->fb_rd_pos, pbi->wait_working_buf);
			return 0;
		}
	} else if (pbi->front_back_mode) {
		if ((pbi->fb_wr_pos != pbi->fb_rd_pos)  || pbi->wait_working_buf)
			return mask;
		else
			return 0;
...
#ifdef NEW_FB_CODE
		ret = ret ? mask : mask & ~(CORE_MASK_HEVC);
#endif
...
static void run_front(struct vdec_s *vdec)
{
	...
	run_count[pbi->index]++;
	vp9_print(pbi, VP9_DEBUG_DUAL_CORE, "run_count = %d\n", run_count[pbi->index]);
	/* pbi->chunk = vdec_prepare_input(vdec); */
#if (!defined SUPPORT_FB_DECODING)
#ifdef NEW_FRONT_BACK_CODE
	if (pbi->front_back_mode) {
		vp9_print(pbi, VP9_DEBUG_DUAL_CORE, "Start FrontEnd Decoding %d\n", pbi->frontend_decoded_count);
		vp9_print(pbi, VP9_DEBUG_DUAL_CORE, "copy loopbuf to next_bk[%d]\n",pbi->fb_wr_pos);
		if (pbi->front_back_mode == 1) {
			copy_loopbufs_ptr(&pbi->next_bk[pbi->fb_wr_pos], &pbi->fr);
			amhevc_reset_f();
		} else
	...
	*/
	...
	if (pbi->front_back_mode == 1 || pbi->front_back_mode == 3) {
		...
		//pyx delete? I think no need
		/*WRITE_VREG(HEVC_DECODE_PIC_BEGIN_REG,
			pbi->frontend_decoded_count);*/
		if (pbi->front_back_mode == 1)
			config_bufstate_front_hw(pbi);
		if (test_debug == 12) {
...
//pyx ucode test
	WRITE_VREG(DECODE_STOP_POS, udebug_flag);
	vp9_print(pbi, PRINT_FLAG_VDEC_DETAIL,
		"%s: start hevc (%x %x %x %x)\n",
		__func__,
		READ_VREG(HEVC_DEC_STATUS_REG),
		READ_VREG(HEVC_MPC_E),
		READ_VREG(HEVC_MPSR),READ_VREG(DECODE_STOP_POS));
...
#ifdef NEW_FB_CODE
static void run_back_fb(struct vdec_s *vdec, void (*callback)(struct vdec_s *, void *, int), void *arg)
{
	struct VP9Decoder_s *pbi =
		(struct VP9Decoder_s *)vdec->private;
	int loadr = 0;
	...
	if (vdec->mc_back_loaded) {
		...
		*/
		...
		if (pbi->front_back_mode == 1) {
			loadr = amhevc_loadmc_ex(VFORMAT_VP9,
					"vp9_back", pbi->fw_back->data);
			...
			loadr = amhevc_loadmc_ex(VFORMAT_VP9,
						"vp9_back", pbi->fw_back->data);
			vp9_print(pbi, 0, "fb_ucode_debug test load\n");
			if (loadr < 0) {
				amhevc_disable();
				vp9_print(pbi, 0, "vp9: the %s back fw loading failed, err: %x\n",
					fw_tee_enabled() ? "TEE" : "local", loadr);
	...
	if (fb_ucode_debug == 1) {
		...
		if (pbi->front_back_mode == 2 || pbi->front_back_mode == 3) {
			WRITE_VREG(pbi->backend_ASSIST_MBOX0_IRQ_REG, 1);
		}
	}
}
#endif
	...
	void (*callback)(struct vdec_s *, void *, int), void *arg)
{
	...
	vp9_print(pbi, VP9_DEBUG_DUAL_CORE, "mask = 0x%lx\n", mask);
	if (test_schedule == 1) {
		mask = CORE_MASK_HEVC;
		pr_err("pyx just run front mask = 0x%lx\n", mask);
	}
	if (pbi->front_back_mode == 0 || (mask & CORE_MASK_HEVC)) {
#endif
	if (vdec->mvfrm)
...
static void vp9_dump_state(struct vdec_s *vdec)
{
	...
	vp9_print(pbi, 0,
		"is_framebase(%d), eos %d, dec_result 0x%x dec_frm %d disp_frm %d run %d not_run_ready %d input_empty %d low_latency %d no_head %d, fb_rd_pos %d, fb_wr_pos %d, wait_working_buf %d, front_back_mode %d\n",
		input_frame_based(vdec),
		pbi->eos,
		pbi->dec_result,
		decode_frame_count[pbi->index],
...
static int ammvdec_vp9_probe(struct platform_device *pdev)
{
...
#ifdef NEW_FB_CODE
	if (get_cpu_major_id() == AM_MESON_CPU_MAJOR_ID_S5) {
		pbi->front_back_mode = front_back_mode;
	} else {
		pbi->front_back_mode = 0;
	}
	if (pbi->front_back_mode) {
		pbi->fb_ifbuf_num = fb_ifbuf_num;
		if (pbi->fb_ifbuf_num > MAX_FB_IFBUF_NUM)
			pbi->fb_ifbuf_num = MAX_FB_IFBUF_NUM;
		pdata->check_input_data = check_input_data;
		pdata->reset = NULL;
		pdata->back_irq_handler = vp9_back_irq_cb;
		pdata->back_threaded_irq_handler = vp9_back_threaded_irq_cb;
	} else
#endif
	pdata->reset = reset;
	...
	if ((debug & IGNORE_PARAM_FROM_CONFIG) == 0 &&
		...
		if (get_config_int(pdata->config, "vp9_double_write_mode",
			...
			pbi->double_write_mode = double_write_mode;
#ifdef OW_TRIPLE_WRITE
		if (get_config_int(pdata->config, "vp9_triple_write_mode",
			&config_val) == 0)
			pbi->triple_write_mode = config_val;
		else
			pbi->triple_write_mode = triple_write_mode;
#endif
...
#ifdef OW_TRIPLE_WRITE
	if (get_cpu_major_id() < AM_MESON_CPU_MAJOR_ID_T3X) {
		if ((pbi->triple_write_mode) || (triple_write_mode) ||
			(pbi->double_write_mode & 0x10000) || (double_write_mode & 0x10000)) {
			double_write_mode &= ~(1 <<16);
			pbi->double_write_mode &= ~(1 <<16);
			triple_write_mode = 0;
			pbi->triple_write_mode = 0;
	...
	if (is_dw_p010(pbi) || is_tw_p010(pbi))
		pbi->endian = HEVC_CONFIG_P010_LE;
	if (endian)
		pbi->endian = endian;
...
#ifdef NEW_FRONT_BACK_CODE
		// pyx need add chip choice
		if (pbi->front_back_mode == 1 || pbi->front_back_mode == 3) {
			struct BuffInfo_s *cur_buf_info = NULL;
			cur_buf_info = &pbi->work_space_buf_store;
			memcpy(cur_buf_info, &amvvp9_workbuff_spec[6],
					sizeof(struct BuffInfo_s));
			init_buff_spec_fb(NULL, cur_buf_info);
			work_buf_size =
				(cur_buf_info->end_adr - cur_buf_info->start_adr
				 + 0xffff) & (~0xffff);
		}
#endif
...
#ifdef NEW_FB_CODE
		if (pbi->front_back_mode)
			vdec_core_release(hw_to_vdec(pbi), CORE_MASK_HEVC | CORE_MASK_HEVC_BACK);
		else
#endif
...
static int __init amvdec_vp9_driver_init_module(void)
{
	...
	register_set_debug_flag_func(DEBUG_AMVDEC_VP9_FB, set_debug_flag);
	...
	vcodec_profile_register_v2("vp9_fb", VFORMAT_VP9, 0);
	INIT_REG_NODE_CONFIGS("media.decoder", &vp9_node,
		"vp9_fb", vp9_configs, CONFIG_FOR_RW);
...
MODULE_PARM_DESC(front_back_mode, "\n amvdec_vp9 front_back_mode\n");

module_param(fb_ucode_debug, uint, 0664);
MODULE_PARM_DESC(fb_ucode_debug, "\n fb_ucode_debug\n");

module_param_array(run_count_back, uint,
		&max_decode_instance_num, 0664);

module_param(test_debug, uint, 0664);
MODULE_PARM_DESC(test_debug, "\n fb_ucode_debug\n");

module_param(test_dbg, uint, 0664);
MODULE_PARM_DESC(test_dbg, "\n fb_ucode_debug\n");

module_param(test_schedule, uint, 0664);
MODULE_PARM_DESC(test_schedule, "\n test_schedule\n");
