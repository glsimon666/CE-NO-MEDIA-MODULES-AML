	...
	if (ret >= 0) {
		...
		if (debug & AV1_DEBUG_BUFMGR) {
			...
			for (i = 0; i < MAX_BUF_NUM; i++) {
				av1_print(hw, 0,
					"mv_Buf(%d) start_adr 0x%lx size 0x%x used %d used_pic_index %d\n",
					i,
...
//#endif
	...
	if (debug) {
		...
		pr_info("dw(%x), Y(0x%lx, %u), C(0x%lx, %u), %dbit yuv\n",
			dw_mode,
			pic->dw_y_adr, pic->luma_size,
			pic->dw_u_v_adr, pic->chroma_size,
			is_dw_p010(hw) ? 10 : 8);

		pr_info("TW(%x), Y(0x%lx, %u), C(0x%lx, %u), %dbit\n",
			tw_mode,
	...
	u8 *hdr10p_data_buf, u32 data_size)
{
	...
	if (debug & AV1_DEBUG_SEI_DETAIL) {
		...
		for (i = 0; i < data_size; i++) {
			av1_print(hw, 0, "%02x ", (BYTE_PTR_CONVERT(usd_rep.v_addr))[i]);
			if (((i + 1) & 0xf) == 0)
				av1_print(hw, 0, "\n");
		}
		av1_print(hw, 0, "\n");
...
static void av1_post_avbcd_task(struct AV1HW_s *hw)
{
	...
	out->done_func = av1_avbc_done_cb;
	...
	av1_print(hw, AV1_DEBUG_BUFMGR,
			...
			" offset %d)\n",
			__func__, hw->mem_map_mode, vf, in->img.data, am_buf->planes[0].addr, in->img.rect.width, in->img.rect.height, in->img.bitdep,
...
static int ammvdec_av1_probe(struct platform_device *pdev)
{
	...
	if (!is_support_triple_write()) {
		if ((hw->triple_write_mode) || (triple_write_mode)) {
			...
			av1_print(hw, 0, "unsupport triple write mode, force disable\n");
		}
	}
	...
	if (!is_support_p010_mode()) {
		if (is_dw_p010(hw)) {
			...
			av1_print(hw, 0, "unsupport dw p010 mode, force disable\n");
		}
		if (is_tw_p010(hw)) {
			triple_write_mode &= ~(1 <<16);
			hw->triple_write_mode &= ~(1 <<16);
			av1_print(hw, 0, "unsupport tw p010 mode, force disable\n");
		}
	}
...
static int __init amvdec_av1_driver_init_module(void)
{
	...
	register_set_debug_flag_func(DEBUG_AMVDEC_AV1_V4L, set_debug_flag);
#ifdef ERROR_HANDLE_DEBUG
	dbg_nal_skip_flag = 0;
