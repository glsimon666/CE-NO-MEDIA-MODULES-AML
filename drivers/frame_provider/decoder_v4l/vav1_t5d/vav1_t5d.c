...
#else
#define MULTI_DRIVER_NAME "ammvdec_av1_t5d_v4l"
#endif
...
struct AV1HW_s {
	 ...
	 * 0:disable rotate  1:dw 0x21  2: dw 3
	 */
	enum T5D_ROTATE_MODE t5d_fg_run_rotate;
	uint8_t t5d_fg_flags;
	uint8_t buff_rotate_in_unfinish;
	uint8_t t5d_rotate_res_ch;
	uint8_t config_next_ref_info_flag_bak;
	uint8_t frame_decoded_bak;
	struct AV1Decoder pbi_bak;
	struct AV1_Common_s common_bak;
...
int check_buff_has_show(struct AV1_Common_s *cm, struct RefCntBuffer_s *frame_buf)
{
	...
	if (hw->t5d_fg_run_rotate ||
		(frame_buf->buf.vf_ref == 0)) {
		ret = 0;
		av1_print(hw, AV1_DEBUG_BUFMGR, "existing buff can use\n");
	} else {
		av1_print(hw, AV1_DEBUG_BUFMGR, "existing buff can't use\n");
	}
	return ret;
}
...
static int v4l_get_free_fb(struct AV1HW_s *hw)
{
	...
	if (hw->t5d_fg_run_rotate == T5D_ROTATE_DW3 &&
		hw->run_rotate_pic) {
		free_pic = hw->run_rotate_pic;
		frame_bufs[free_pic->index].ref_count = 1;
		return free_pic->index;
	}
...
static struct BuffInfo_s aom_workbuff_spec[WORK_BUF_SPEC_NUM] = {
	{ //8M bytes
		...
		.cdf_buf		= {.buf_size = 0x80000},  // for context store/load 1024 x 256bits x 16 = 512K bytes x 16 = 0x80000
		.gmc_buf		= {.buf_size = 0x800},  // for gmc_parameter store/load 128 bytes x 16 = 2K bytes = 0x800
		...
		.fgs_table		= {.buf_size = FGS_TABLE_SIZE * FRAME_BUFFERS}, // 512x128bits per frame
#ifdef AOM_AV1_MMU
...
#ifdef MV_USE_FIXED_BUF
		.mpred_mv		= {.buf_size = MAX_ONE_MV_BUFFER_SIZE_4K_TM2REVB * FRAME_BUFFERS}, // .buf_size = 0x100000*16, 4k2k => 0x100000 per buffer 4096x2304 =>0x120000 per buffer
#endif
		.rpm 			= {.buf_size = 0x80 * 2},
		.lmem			= {.buf_size = 0x400 * 2},
	},
	{
		...
		.sao_up			= {.buf_size = 0x0}, // SAO UP STORE AREA - Max 640(10240/16) LCU, each has 16 bytes total 0x2800 bytes
		.swap_buf		= {.buf_size = 0x800}, // 256cyclex64bit = 2K bytes 0x800 (only 144 cycles valid)
		.cdf_buf		= {.buf_size = 0x80000}, // for context store/load 1024 x 256bits x 16 = 512K bytes x 16 = 0x80000
		.gmc_buf		= {.buf_size = 0x800}, // for gmc_parameter store/load 128 bytes x 16 = 2K bytes = 0x800
		.scalelut		= {.buf_size = 0x0}, //0x8000  support up to 32 SCALELUT 1024x32 = 32Kbytes (0x8000)
...
#ifdef MV_USE_FIXED_BUF
		.mpred_mv		= {.buf_size = MAX_ONE_MV_BUFFER_SIZE_8K_TM2REVB * FRAME_BUFFERS}, //buf_size = 0x100000*16, 4k2k => 0x100000 per buffer  4096x2304 => 0x120000 per buffer
#endif
		.rpm			= {.buf_size = 0x80 * 2},
		.lmem			= {.buf_size = 0x400 * 2},
	}
...
static void config_dblk_hw(struct AV1HW_s *hw)
{
	...
	*/
	...
	if (cm->cur_frame <= 0) {
		WRITE_VREG(enable_swap?HEVC_ASSIST_SCRATCH_3:HEVC_ASSIST_SCRATCH_8, //AOM_AV1_CDF_BUFFER_W
			buf_spec->cdf_buf.buf_start);
				...
				struct PIC_BUFFER_CONFIG_s *pic_config)
{
	...
	if (vf) {
		...
		if (hw->t5d_fg_run_rotate) {
			vf->compHeadAddr = pic_config->header_dw_adr;
			vf->fgs_valid = fgs_valid;
			av1_print(hw, AOM_DEBUG_VFRAME,
				"Use dw mmu for display header_dw_adr:0x%x fgs_valid:%d\n",
				pic_config->header_dw_adr, vf->fgs_valid);
		}
...
static int check_fg_change(struct AV1HW_s *hw)
{
	...
	if (use_dw_mmu) {
		av1_print(hw, AV1_DEBUG_BUFMGR, "%s use_dw_mmu:1\n", __func__);
		return 1;
	}

	if (!hw->film_grain_present || disable_fg) {
		av1_print(hw, AV1_DEBUG_BUFMGR, "%s film_grain_present:%d disable_fg:%d\n",
			__func__, hw->film_grain_present, disable_fg);
		return 0;
	}
...
static void av1_bufmgr_backup(struct AV1HW_s *hw)
{
	if (!hw || !hw->pbi)
		return;

	memcpy((void *)&hw->pbi_bak, (void *)hw->pbi, sizeof(struct AV1Decoder));
	memcpy((void *)&hw->common_bak, (void *)&hw->common, sizeof(struct AV1_Common_s));
	memcpy((void *)&hw->av1_buffer_pool_bak, (void *)&hw->av1_buffer_pool, sizeof(struct BufferPool_s));
}

static void av1_bufmgr_restore(struct AV1HW_s *hw)
{
	int i;
	if (!hw || !hw->pbi)
		return;

	if (hw->t5d_rotate_res_ch) {
		hw->t5d_rotate_res_ch = 0;
		hw->common_bak.width = hw->common.width;
		hw->common_bak.height = hw->common.height;
	}
	memcpy((void *)hw->pbi, (void *)&hw->pbi_bak, sizeof(struct AV1Decoder));
	memcpy((void *)&hw->common, (void *)&hw->common_bak, sizeof(struct AV1_Common_s));

	for (i = 0; i < FRAME_BUFFERS; i++) {
		hw->av1_buffer_pool_bak.frame_bufs[i].buf = hw->av1_buffer_pool.frame_bufs[i].buf;
		memcpy((void *)&hw->av1_buffer_pool.frame_bufs[i], (void *)&hw->av1_buffer_pool_bak.frame_bufs[i], sizeof(struct RefCntBuffer_s));
	}
}

static void av1_decoder_restore(struct AV1HW_s *hw)
{
	if (!hw)
		return;

	hw->config_next_ref_info_flag = hw->config_next_ref_info_flag_bak;
	hw->frame_decoded = hw->frame_decoded_bak;

	av1_bufmgr_restore(hw);
}

static void av1_decoder_backup(struct AV1HW_s *hw)
{
	if (!hw)
		return;

	hw->config_next_ref_info_flag_bak = hw->config_next_ref_info_flag;
	hw->frame_decoded_bak = hw->frame_decoded;

	av1_bufmgr_backup(hw);
}
...
static irqreturn_t vav1_isr_thread_fn(int irq, void *data)
{
		...
		if (hw->m_ins_flag) {
			...
			shift_byte + 2 < hw->data_size) { //obu head 1 byte, obu payload 1 byte
				if (enable_single_slice == 1) {
					if (hw->t5d_fg_run_rotate == T5D_ROTATE_DW21) {
						hw->t5d_fg_run_rotate = T5D_ROTATE_DW3;
						hw->double_write_mode = 3;
						hw->process_busy = 0;
						av1_decoder_restore(hw);
						if (hw->buff_rotate_in_unfinish) {
	...
	if (obu_type == OBU_SEQUENCE_HEADER) {
		...
		if ((hw->film_grain_present || use_dw_mmu) &&
			!hw->t5d_fg_run_rotate &&
			hw->t5d_fg_flags) {
			hw->t5d_fg_run_rotate = T5D_ROTATE_DW21;
		}
...
static void  av1_decode_ctx_reset(struct AV1HW_s *hw)
{
	...
	hw->aml_buf		= NULL;
	hw->dec_result = DEC_RESULT_NONE;
	hw->t5d_fg_run_rotate = T5D_DISABLE_ROTATE;
}
...
static int ammvdec_av1_probe(struct platform_device *pdev)
{
	...
	pr_debug("ammvdec_av1_t5d_probe\n");
...
static int __init amvdec_av1_driver_init_module(void)
{
	...
	if (get_cpu_major_id() == AM_MESON_CPU_MAJOR_ID_T5D) {
		use_t5d_driver_set(true);
		vcodec_profile_register_v2("AV1-V4L", VFORMAT_AV1, 1);
	}
...
module_param(fgs_valid, uint, 0664);
MODULE_PARM_DESC(fgs_valid, "\n fgs_valid\n");
